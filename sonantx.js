...
function SoundWriter (instr, n, rowLen) {
  ...
  this.write = function (lchan, rchan, from) {
    ...
    while (j < instr.env_attack + instr.env_sustain + instr.env_release && c < lchan.length) {
      // LFO
      const lfor = osc_lfo(j * lfoFreq) * instr.lfo_amt / 512 + 0.5

      // Envelope
      let e = 1
      if (j < instr.env_attack) { e = j / instr.env_attack } else if (j >= instr.env_attack + instr.env_sustain) { e -= (j - instr.env_attack - instr.env_sustain) / instr.env_release }
      ...
      c1 += t
      let rsample = osc1(c1) * instr.osc1_vol
    ...
    if (c < lchan.length) { return true }
    return false
  }
};

const TrackGenerator = function (audioCtx, instr, rowLen, endPattern) {
...
const MusicGenerator = function (audioCtx, song) {
  this.audioCtx = audioCtx
  this.song = song
...
MusicGenerator.prototype.start = function (when) {
  if (when === undefined) { this.chain[0].start() } else { this.chain[0].start(when) }
}
MusicGenerator.prototype.connect = function (target) {
